#!/bin/bash
# t3-noflit.sh - RP<->USP link without flit mode (fail-fast expected).
# QEMU mirrors the child's flit status into the port, so the only way
# to model a non-flit link is at the switch upstream side; the BI
# prerequisite then fails region attach before UIO commit - both gates
# lead to the same fail-closed outcome.
#
#   pxb-cxl (cxl.1)
...
launch_qemu \
	...
	-device cxl-upstream,bus=rp0,id=us0,$PORT_SVC,x-256b-flit=off \
